...
export default function CategoriesPage() {
  ...
  const openCreate = () => { setEditing(null); setForm({ name: '', slug: '', icon: '', sortOrder: '0' }); setShowModal(true); };
  const openEdit = (c: Category) => { setEditing(c); setForm({ name: c.name, slug: c.slug, icon: c.icon || '', sortOrder: String(c.sortOrder) }); setShowModal(true); };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50">
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl w-full max-w-sm">
            ...
            <div className="p-5 space-y-4">
              ...
              <div><label className="block text-xs font-medium text-gray-700 dark:text-slate-300 mb-1.5">Icon (emoji)</label><input type="text" value={form.icon} onChange={e => setForm(f => ({ ...f, icon: e.target.value }))} className={inp} placeholder="🥦" /></div>
