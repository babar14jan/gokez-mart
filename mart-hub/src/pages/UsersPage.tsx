  ...
  return (
    <div className="space-y-4">
      ...
      <div className="page-card">
        {/* Desktop header */}
        <div className="hidden sm:grid grid-cols-12 gap-3 px-4 py-2.5 bg-gray-50 dark:bg-slate-700 border-b border-gray-100 dark:border-slate-600 text-[11px] font-semibold text-gray-500 uppercase tracking-wider">
        ...
        {users.length === 0 ? (
          ...
          <div key={u.id} className="grid grid-cols-1 sm:grid-cols-12 gap-1 sm:gap-3 px-4 py-3 border-b border-gray-50 dark:border-slate-700 last:border-0 hover:bg-gray-50 dark:hover:bg-slate-700/50 items-center">
            ...
            <div className="sm:hidden flex items-center justify-between">
              <div>
                <div className="flex items-center gap-2">
                  <p className={`text-sm font-semibold ${u.isActive ? 'text-gray-900 dark:text-white' : 'text-gray-500 dark:text-slate-400 line-through'}`}>{u.name || u.username}</p>
                ...
                <p className="text-xs text-gray-500 dark:text-slate-400">@{u.username} · {storeName(u.storeId)}</p>
              </div>
              <div className="flex gap-1">
                ...
                <button onClick={() => openEdit(u)} className="p-1.5 rounded-lg text-gray-500 hover:text-indigo-600 hover:bg-indigo-50 dark:hover:bg-indigo-900/20"><Pencil className="w-4 h-4" /></button>
                {u.id !== currentUserId && <button onClick={() => setConfirmDelete(u)} className="p-1.5 rounded-lg text-gray-500 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20"><Trash2 className="w-4 h-4" /></button>}
              </div>
            </div>
            {/* Desktop */}
            <div className="hidden sm:block col-span-3">
              <p className={`text-xs font-semibold ${u.isActive ? 'text-gray-900 dark:text-white' : 'text-gray-500 dark:text-slate-400 line-through'}`}>{u.name || u.username}</p>
              <p className="text-[10px] text-gray-500 dark:text-slate-400">@{u.username}</p>
            </div>
                ...
                <div className="flex flex-wrap gap-1">
                  {userStores[u.id].map((s: any) => (
                    <div key={s.id} className="flex items-center gap-1">
                      <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-medium ${s.isActive ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400' : 'bg-gray-100 text-gray-500 line-through'}`}>{s.name}</span>
            ...
            <div className="hidden sm:block col-span-1 text-[10px] text-gray-500 dark:text-slate-400">
            ...
            <div className="p-5 space-y-4">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <button type="button" onClick={() => setShowPw(p => !p)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-500 hover:text-gray-600">
      ...
      {confirmDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50">
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl w-full max-w-sm p-6 text-center">
            ...
            <p className="text-xs text-gray-500 dark:text-slate-400 mb-5">This cannot be undone.</p>
